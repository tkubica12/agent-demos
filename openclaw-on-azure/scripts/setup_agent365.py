...
LOCAL_METADATA = "openclaw-agent365-identifiers.json"
...
def customize_manifest(workspace: Path) -> Path:
    ...
    manifest["name"] = {
        "short": "OpenClaw",
        "full": "OpenClaw on Azure",
    }
    manifest["description"] = {
        "short": "Chat with OpenClaw running in ACA Sandboxes.",
        "full": (
            "OpenClaw on Azure exposes the OpenClaw Gateway through a governed Agent 365 identity. "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Prepare and optionally run Agent 365 registration for the deployed OpenClaw bridge endpoint."
    )
