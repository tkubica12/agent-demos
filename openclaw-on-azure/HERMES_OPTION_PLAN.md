...
This plan describes how the current OpenClaw-on-Azure project should become `autopilots-on-azure`: a shared Azure, Teams, and Agent 365 hosting pattern that can run both OpenClaw and Nous Research Hermes as selectable autopilot runtimes.
...
The current folder should be renamed from `openclaw-on-azure` to `autopilots-on-azure`. Within it, split common code from runtime-specific code.
...
- Rename `sandbox_gateway.py` to `sandbox_runtime.py`.
