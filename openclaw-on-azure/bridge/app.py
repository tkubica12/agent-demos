...
def should_observe_unmentioned_messages() -> bool:
    return env_optional("OPENCLAW_TEAMS_OBSERVE_UNMENTIONED", "true").lower() in {"1", "true", "yes"}
...
def env_int(name: str, default: int) -> int:
    try:
        return int(env_optional(name, str(default)))
    except ValueError:
        return default
...
def message_mentions_bot_name(activity: MessageActivity, message: str | None = None) -> bool:
    ...
    aliases = {
        ...
        env_optional("OPENCLAW_TEAMS_NAME", "OpenClaw").lower(),
    }
...
async def run_openclaw_for_teams(
    ctx: ActivityContext,
    *,
    conversation_id: str,
    session_key: str,
