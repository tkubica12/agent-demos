...
| 5. Quality and safety | Complete with preview limitations | 30-case evaluation, optimizer review, baseline promotion, trace metrics, cloud red-team submission, and a portal-visible six-attack local red-team run are complete |
| Portal experiences | Complete with two external blockers | Stored Completions, persistent Memory, custom Guardrail, Content Understanding, rich PDFs, Foundry IQ, and sequential workflow are live; the trace curator and secretless Work IQ integration are blocked upstream |

Current immutable assets:

- Hosted Agent `foundry-showcase-main`, active and only retained version 27;
...
Provision the portal experience resources and populate the inspectable examples. The commands resolve every required Terraform input from the existing Foundry project and its connected observability resources:
...
$appInsights = az monitor app-insights component show `
  --app step06-foundry-observability-evals-ai `
  --resource-group $foundryResourceGroup | ConvertFrom-Json
...
$env:TF_VAR_application_insights_id = $appInsights.id
$env:TF_VAR_log_analytics_workspace_id = $appInsights.workspaceResourceId
...
$env:MEMORY_SCOPE = "$($account.tenantId)_$portalUserObjectId"
...
azd ai agent eval run `
  --config eval.yaml `
  --name foundry-showcase-v27-final `
  --no-prompt `
...
- The Foundry trace curator has Reader access to Application Insights and its workspace, and version 27 emits content-rich user, assistant, and tool events. Generation by agent name, version, Hosted Agent GUID, and telemetry ID still returns `DataGenerationJobNoTracesFound`. Failed jobs cannot currently be removed because the preview API returns `unexpectedEntityState`.
