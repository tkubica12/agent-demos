...
Evolve the OpenClaw runtime path inside Autopilots on Azure from a directly accessed ACA Sandbox gateway into a Teams / Agent 365 agent experience:
...
generated.app.auto.tfvars.json         app/runtime identity and token values from setup/pairing script
```
...
1. Script runs ACR remote builds for:
   - `runtimes\openclaw\Dockerfile` -> OpenClaw runtime image.
...
- `scripts\setup_app_tfvars.py` creates OpenClaw runtime identity inputs and writes app tfvars.
...
- `scripts\sandbox_runtime.py` reusable sandbox lifecycle code.
- `scripts\sandbox_run_runtime.py` wrapper to run/wake OpenClaw in ACA Sandbox.
...
- Status: implemented and deployed in the current environment. The Teams package has been refreshed at `.local\ehvw\teams\openclaw-autopilot-teams.zip`; upload it to Teams before testing group chat and channel scopes.
