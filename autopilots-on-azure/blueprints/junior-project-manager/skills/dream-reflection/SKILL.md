...
3. Update private memory or `local/private-cache.md` only for the first two classes. Never copy those details into a transferable record and never write persistent notes under `/root`.
...
Accepted transferable records are materialized into `skills/hot-learning` so this worker benefits before fleet consolidation. Do not edit that generated skill directly.
