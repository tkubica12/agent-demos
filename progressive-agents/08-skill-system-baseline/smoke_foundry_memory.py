...
def ensure_memory_store(client: AIProjectClient, name: str) -> dict[str, Any]:
    ...
    created = client.beta.memory_stores.create(
        ...
        description="Step 08 user profile, summaries, and procedural memory.",
    )
    return model_dump(created)


def main() -> None:
    memory_store_name = os.getenv("MEMORY_STORE_NAME", "step-08-skill-system-baseline")
    scope = os.getenv("MEMORY_SCOPE_TEST", f"step08-smoke-{uuid.uuid4()}")
    chat_model = required_env("MEMORY_STORE_CHAT_MODEL_DEPLOYMENT_NAME")
    marker = f"step08-foundry-memory-{uuid.uuid4()}"
    ...
    for prompt in (
        f"Remember that my Step 08 smoke marker is {marker}.",
        f"Use the memory tool to remember exactly this fact: my Step 08 smoke marker is {marker}.",
        f"Please store this in memory now: Step 08 smoke marker = {marker}.",
    ):
