...
Goal: keep the step 07 Foundry Memory/profile agent and add a reusable
Microsoft Agent Framework skill baseline using Agent Skills-compatible
`SKILL.md` packages. Keep BFF thin. Agent owns memory and skill selection.
...
```text
skills/base/
  support-style/SKILL.md
  escalation-policy/SKILL.md
  escalation-policy/references/escalation-matrix.md
  profile-update-policy/SKILL.md
```

The hosted Agent Framework agent wires `SkillsProvider.from_paths(...)` as a
context provider. MAF advertises only skill names/descriptions up front. The
model must call `load_skill` to retrieve the full `SKILL.md` and
`read_skill_resource` to load referenced resources such as the escalation
matrix. Script skills are intentionally not enabled in this baseline.

Distribution paths to understand:

| Path | Use |
| --- | --- |
| Local file skills | Current implementation. Checked-in `SKILL.md` packages are repeatable and easy to test. |
| Foundry Skills download | Future promotion path. Upload/version skills centrally, then download selected packages at hosted-agent startup. |
| Foundry toolbox MCP skills | Future/shared-client path. Attach skills to a toolbox version and consume them through `MCPSkillsSource` over the toolbox MCP endpoint. |
...
The memory tool points at MEMORY_STORE_NAME, default step-08-skill-system-baseline.
...
$env:MEMORY_STORE_NAME = "step-08-skill-system-baseline"
...
$env:LOCAL_SKILLS_PATH = ".\skills\base"
uv sync
uv run python main.py
```

Smoke local skill packages:
...
$env:MEMORY_STORE_CHAT_MODEL_DEPLOYMENT_NAME = "gpt-5.4-mini"
$env:MEMORY_STORE_EMBEDDING_MODEL_DEPLOYMENT_NAME = "text-embedding-3-large"
uv run python scripts\setup_foundry_memory.py --name step-08-skill-system-baseline
```
...
```powershell
$env:FOUNDRY_PROJECT_ENDPOINT = "https://<account>.services.ai.azure.com/api/projects/<project>"
.\scripts\Provision-Step08.ps1
```

Deploy Hosted Agent:

```powershell
azd deploy -C progressive-agents\08-skill-system-baseline
```

Test deployed cross-session Foundry Memory:

```powershell
cd progressive-agents\08-skill-system-baseline

$write = Join-Path $env:TEMP "step08-memory-write.json"
@'
{
  "message": "Please remember this durable Step 08 fact in Foundry Memory: my raw-inspectable codename is silver-prism-0628.",
  "stream": false,
...
azd ai agent invoke step-08-skill-system-baseline --protocol invocations --new-session --user-isolation-key raw-memory-user --chat-isolation-key silver-write -f $write

Start-Sleep -Seconds 35

$read = Join-Path $env:TEMP "step08-memory-read.json"
@'
{
...
azd ai agent invoke step-08-skill-system-baseline --protocol invocations --new-session --user-isolation-key raw-memory-user --chat-isolation-key silver-read -f $read
```
...
```text
MAF SkillsProvider loads checked-in SKILL.md packages
support-style, escalation-policy, and profile-update-policy validate
escalation-policy resource can be read through the MAF skill resource API
