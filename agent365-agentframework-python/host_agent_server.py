...
# Compatibility shim: must be imported before microsoft_agents_a365 packages
import agent_framework_compat  # noqa: F401
...
class GenericAgentHost:
    ...
    def __init__(self, agent_class: type[AgentInterface], *agent_args, **agent_kwargs):
        ...
        try:
            ...
            # No SERVICE_CONNECTION configured — provide a dummy for anonymous/local dev
            logger.info("🔓 No SERVICE_CONNECTION found; using anonymous connection manager")
            dummy_auth = AgentAuthConfiguration(
                client_id="anonymous",
                tenant_id="anonymous",
                client_secret="anonymous",
            )
            self.connection_manager = MsalConnectionManager(
                connections_configurations={"SERVICE_CONNECTION": dummy_auth}
            )
